                ...
                <div class="card-body">
                    ...
                    <table class="table table-striped table-hover" id="tableExport" style="width:100%;">
                        ...
                        <?php
                            ...
                            AND customers.companyId=offices.companyId AND offices.id = customers.offId AND offices.status='Rent' AND `reserveStatus`='Cancelled' ORDER BY customers.id DESC") or die(mysqli_error($conn));
                                ...
                                while ($rows = mysqli_fetch_array($query)) {
                                   ?>
                                        <tr>
                                            ...
                                            <td><?= $rows['checkin'];?></td>
                                            <td><?= $rows['checkout'];?></td>
                                            <td><label class="text-danger"><?= $rows['reserveStatus']?></label></td>
                                            <td>
                                            <button onClick="updateResStatus('<?= $rows['reserveId']?>', '<?= $rows['offId']?>')" style="color:white;" class="btn btn-flat btn-info btn-sm">Re-Activate</button>
                                            </td>
...
<script type="text/javascript">
  function removeBooking(id) {
    var com = confirm("Do you really want to remove this reservation?");
    if (com) {
